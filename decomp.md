...
```
**Answer**
- `this.handleClick1()` is executed as component is mounted.
- `this.handleClick1` does not work because a function passed in `onClick` of Click 2 button needs to have event parameter. 
- `this.handleClick2` will work as it is bind with handleClick1 so it will trigger hancleClick1 method.
- `this.handleClick3` will work as it will call a variable which has method that trigger alert. 
...
```
**Answer**
I think memoize method will not be resolved and the test will not run.